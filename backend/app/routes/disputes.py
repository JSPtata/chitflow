...
def raise_dispute(
    ...
    if (
        round_obj.current_state
        != "CHALLENGE_OPEN"
    ):
        raise HTTPException(
            status_code=400,
            detail=(
                "Disputes can only be raised "
...
def get_round_disputes(
    round_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """
    Return disputes belonging to a round.

    Access is allowed when the current user:
    - is an active member of the chit group, or
    - is the creator of the chit group.
    """
...
def resolve_dispute(
    ...
    dispute.resolution = (
        resolution_data.resolution  # type: ignore
    )
